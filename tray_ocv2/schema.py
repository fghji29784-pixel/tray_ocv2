...
# ---------------------------------------------------------------------------
# ⚠️⚠️ 좌표계 방향 — 미확정. 반드시 확인할 것.
# ---------------------------------------------------------------------------
# 기존 두 문서가 서로 모순된다:
#   (가) 전역 메모리      : 알파벳 A~L = **열(column)**, 숫자 01~12 = 행(row)
#   (나) tray_ocv 저장소  : 알파벳 A~L = **행(row)**,    숫자 01~12 = 열(column)
#
# 트레이가 12×12 정사각이라 **틀려도 shape 검사에 안 걸린다.** 전치(transpose)된 채로
# 모든 그림과 통계가 조용히 계산된다.
#
# 그리고 이건 치명적이다 — 팬 배열이 **행 방향 3밴드 / 열 방향 4-5-4** 라서,
# 축이 뒤집히면 F1(행 프로파일)·F2(밴드별 열 프로파일)의 결론이 **정반대로 나온다.**
#
# 확인 방법:
#   1) 데이터의 `Cell No` ↔ `Cell 위치` 대응으로 두 컬럼 간 규칙은 확정할 수 있다.
#      (예: Cell No 2 가 'A02' 면 숫자축을 따라 증가, 'B01' 이면 알파벳축을 따라 증가)
#   2) 그러나 **어느 축이 충방전기 기준 "세로(팬 밴드 방향)"인지**는 데이터로 알 수 없다.
#      → **설비 담당자 확인 필수.** 트레이를 충방전기에 넣는 방향 기준.
#
# 확인되면 ALPHABET_AXIS 만 바꾸면 전체가 따라온다.
ALPHABET_AXIS = "row"   # "row" | "col"  ← ⚠️ 임시 기본값. 확인 후 확정할 것.

#: 팬 밴드(4-5-4)가 놓인 축. 도면 기준 '행 방향으로 3밴드'.
#: ALPHABET_AXIS 가 확정되어야 이 축이 데이터의 어느 컬럼인지 결정된다.
FAN_BAND_AXIS = "row"
